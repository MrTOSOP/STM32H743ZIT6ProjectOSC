...
//LCD Init
  ...
  LCD_Clear();
  LCD_SetAsciiFont(&ASCII_Font20);//Font20_Size 
  LCD_ShowNumMode(Fill_Space);
  LCD_SetColor(LCD_GREEN);
  TIM17->PSC = 9600-1;
  TIM17->ARR = 5000-1;
  LCD_Show_Basic();
  HAL_TIM_Base_Start_IT(&htim17);//TIM17_Control_LCD_FPS
//FFT
  fft_init();
//Peripherals Init
  Wave_Value_G();
  ...
  while (1)
  {
      if(TIM17_EXTI_FLAG == 1)
      {
          LCD_Show_Freq_Duty();
      }
      
      
      if((adc_Completed_flag == 1) &&(Flag_GPIOF_PIN_7 == 0))
      {
          adc_Completed_flag = 0;
          Print_ADC_Value();
      }
      
      if((adc_Completed_flag == 1) && Flag_GPIOF_PIN_7 == 1 &&(TIM17_EXTI_FLAG == 1))
      {
          adc_Completed_flag = 0;
          calculate_fft();
